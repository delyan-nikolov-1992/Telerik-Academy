    ...
    public class FigureController
    {
        ...
        protected virtual void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                ...
                case "area":
                    {
                        string result = this.currentFigure is IAreaMeasurable
                            ? string.Format("{0:0.00}", ((IAreaMeasurable)this.currentFigure).GetArea())
                            : "undefined";
                ...
                case "volume":
                    {
                        string result = this.currentFigure is IVolumeMeasurable
                            ? string.Format("{0:0.00}", ((IVolumeMeasurable)this.currentFigure).GetVolume())
                            : "undefined";
                        Console.WriteLine(result);
                        break;
                    }
                case "normal":
                    {
                        string result = this.currentFigure is IFlat
                            ? ((IFlat)this.currentFigure).GetNormal().ToString()
                            : "undefined";
                        Console.WriteLine(result);
                        break;
                    }
