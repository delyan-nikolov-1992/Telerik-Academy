...
namespace Infestation
{
    abstract public class Unit
    {
        ...
        public virtual Interaction DecideInteraction(IEnumerable<UnitInfo> units)
        {
            ...
            if (this is Queen || this is Parasite)
            {
                IList<UnitInfo> listOfInfectibles = new List<UnitInfo>();

                foreach (var unit in units)
                {
                    if (InfestationRequirements.RequiredClassificationToInfest(unit.UnitClassification)
                        .Equals(this.UnitClassification))
                    {
                        listOfInfectibles.Add(unit);
                    }
                }

                var orderedByHealth = listOfInfectibles.OrderBy(u => u.Health);

                return new Interaction(new UnitInfo(this), listOfInfectibles[0], InteractionType.Infest);
            }
