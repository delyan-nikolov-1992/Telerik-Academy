...
class SubtractAndMultiplyPolynomials
{
    static void Main()
    {
        try
        {
            ...
            if (firstSize >= 0 && secondSize >= 0)
            {
                ...
                result = Subtract(firstPolynomial, secondPolynomial);
    ...
    static double[] Subtract(double[] firstPolynomial, double[] secondPolynomial)
    {
        double[] result;
        int smallerLength;
        double[] biggerPolynomial;
        ...
        for (int i = smallerLength; i < result.Length; i++)
        {
            if (smallerLength == firstPolynomial.Length)
            {
                result[i] = biggerPolynomial[i] * -1;
            }
            else
            {
                result[i] = biggerPolynomial[i];
    ...
    static double[] Multiply(double[] firstPolynomial, double[] secondPolynomial)
    {
        double[] result = new double[firstPolynomial.Length + secondPolynomial.Length - 1];
